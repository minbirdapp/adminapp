...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileForm) {
        if (imageInput) {
            imageInput.addEventListener('change', function() {
                ...
                if (file) {
                    ...
                    reader.onload = e => { if (profileImage) profileImage.src = e.target.result; };
        ...
        profileForm.addEventListener('submit', e => {
            ...
            if (!firstName.value.trim()) { valid = false; setError(firstName, 'First name is required.'); }
            if (!lastName.value.trim()) { valid = false; setError(lastName, 'Last name is required.'); }
            if (!timezone.value.trim()) { valid = false; setError(timezone, 'Please select a timezone.'); }
            if (!valid) e.preventDefault();
        });
    }
    ...
    if (inviteForm) {
        inviteForm.addEventListener('submit', e => {
            ...
            if (!firstName.value.trim()) { valid = false; setError(firstName, 'First name is required.'); }
            if (!lastName.value.trim()) { valid = false; setError(lastName, 'Last name is required.'); }
            if (!role.value.trim()) { valid = false; setError(role, 'Please select a role.'); }
            if (!email.value.trim()) { valid = false; setError(email, 'Email is required.'); }
            else if (!emailPattern.test(email.value.trim())) { valid = false; setError(email, 'Enter a valid email.'); }
            if (status.value === '') { valid = false; setError(status, 'Please select a status.'); }
            if (!valid) e.preventDefault();
        });
    }
